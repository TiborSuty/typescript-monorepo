...
import {DefaultFoundationTheme} from '../../utils/foundationTheme';
...
export function DeviceProvider(props: DeviceProviderProps) {
  ...
  const [currentDevice, setCurrentDevice] = useState<Device>(() => getCurrentDevice(theme));

  useLayoutEffect(() => {
    const mediaQueryLists = toPairs(theme.devices).map(([device, mediaQuery]) => ({
      device,
    ...
    mediaQueryLists.forEach((deviceQueryList) => {
      if (isAddEventListenerSupported) {
        deviceQueryList.queryList?.addEventListener?.('change', deviceQueryList.listener);
      } else {
    ...
    return () => {
      mediaQueryLists.forEach((deviceQueryList) => {
        if (isAddEventListenerSupported) {
          deviceQueryList.queryList?.removeEventListener?.('change', deviceQueryList.listener);
        } else {
  ...
  return <DeviceContextProvider value={currentDevice}>{props.children}</DeviceContextProvider>;
}
